...
class Item(BaseModel):
    ...
    operation: str
    edges: str
@app.get("/NetworkManage/{TF_like}/{TF_retweet}/{TF_comment}/{TF_view}/{TF_collect}/{TF_degree}/{TF_att}/{TF_pcomment}/{TF_sample}/{TF_leader}/{TF_media}/{cnt_dc}/{cnt_eg}/{cnt_t}/{cnt_l}/{cnt_sc}/{cnt_sd}/{cnt_ai}/{cnt_ep}/{cnt_bm}/{cnt_c}/{cnt_s}/{operation}/{edges}")
def NetworkManage(TF_like: bool, TF_retweet: bool, TF_comment: bool, TF_view: bool, TF_collect: bool,TF_degree: bool, TF_att: bool, TF_pcomment: bool, TF_sample:bool, TF_leader: bool,TF_media: bool,cnt_dc: float, cnt_eg: float, cnt_t: float, cnt_l: float, cnt_sc: float, cnt_sd: float, cnt_ai: float, cnt_ep: float, cnt_bm: float, cnt_c: float,cnt_s:float, operation: str, edges: str):
    """
    生成网络图数据
    ...
    params = [cnt_dc, cnt_eg, cnt_t, cnt_l, cnt_sc, cnt_sd, cnt_ai, cnt_ep, cnt_bm, cnt_c,cnt_s]
    ...
    if sum(params_normalized) == 1:                                                 
        try:
            print(111)
            print(df.shape[0])  # Ensure df is defined correctly
            print(222)
            prof_color_mapping = {
                "Arts": "red",
                "Business": "blue",
